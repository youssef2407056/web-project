(function () {
  ...
  function toggleFavoriteId(id) {
    if (window.noirToggleFavoriteId) return window.noirToggleFavoriteId(id);
  ...
  function updateQvFavButton(btn, id) {
    if (!btn || !id) return;
    var on = window.noirIsFavoriteId ? window.noirIsFavoriteId(id) : getFavorites().includes(id);
  ...
  function open(productId) {
    ...
    modal.setAttribute('data-product-id', String(productId));
    qvQtyVal = 1;
    ...
    addBtn.onclick = function () {
      ...
      if (window.addToCart) {
        var added = window.addToCart(p.name, o.price, p.image, String(o.ml), p.id, {
  ...
  window.addEventListener('noir-currency-change', function () {
    ...
    var pid = modal.getAttribute('data-product-id');
