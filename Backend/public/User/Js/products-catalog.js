...
(function () {
  function isExpressApp() {
    var path = (typeof window !== 'undefined' && window.location.pathname || '').replace(/\\/g, '/');
  ...
  if (typeof window !== 'undefined' && typeof window.noirUserHtmlPrefix !== 'string') {
    ...
    window.noirUserHtmlPrefix = (__path === '/' || /(^|\/)home\.html$/.test(__path)) ? 'User/Html/' : '';
  }
  ...
  function htmlPage(name) {
    if (isExpressApp()) {
      var routes = {
        'Shopall.html': '/shop',
        'Men.html': '/shop?category=men',
        'Women.html': '/shop?category=women',
        'Unisex.html': '/shop?category=unisex',
        'aboutus.html': '/about',
        'contactus.html': '/contact',
        'chanel.html': '/shop?brand=chanel',
        'dior.html': '/shop?brand=dior',
        'boss.html': '/shop?brand=boss',
        'login.html': '/auth/login',
        'signup.html': '/auth/signup',
        'checkout.html': '/checkout',
        'checkout-thanks.html': '/checkout/thank-you',
        'favorites.html': '/shop'
      };
      if (routes[name]) return routes[name];
      if (name.indexOf('product.html') === 0) {
        var q = name.indexOf('?') >= 0 ? name.slice(name.indexOf('?') + 1) : '';
        var params = new URLSearchParams(q);
        var shop = params.get('shop');
        var pid = params.get('id');
        if (shop != null && shop !== '') {
          return '/products/shop-' + encodeURIComponent(shop);
        }
        if (pid) {
          return '/products/' + encodeURIComponent(String(pid));
        }
        return '/shop';
      }
      return '/';
  ...
  function shopImg(path) {
    ...
    if (path.startsWith('../')) {
      if (isExpressApp()) return '/User/Images/' + path.replace(/^\.\.\/Images\//, '');
      return path;
    }
    if (isExpressApp() || (typeof window !== 'undefined' && window.noirUserHtmlPrefix)) {
      return (isExpressApp() ? '/User/Images/' : 'User/Images/') + path;
    }
    return '../Images/' + path;
  }
  ...
  var SHOP_ROWS = [
    ...
    { name: 'Nicarage', img: '/uploads/1780345263424-815678987.jpeg', line: 'Unisex Eau de Parfum', p100: 680 },
  ...
  var LEGACY_ALIAS = {};
  var catalogUsesDatabase = false;

  function imageFileKey(pathLike) {
  ...
  function findApiProductForShopRow(row, apiProducts) {
    var rowImg = imageFileKey(row.img);
    var rowName = String(row.name || '').trim().toLowerCase();
    for (var i = 0; i < apiProducts.length; i++) {
      var p = apiProducts[i];
      if (!p) continue;
      if (imageFileKey(p.image) === rowImg) return p;
    }
    for (var j = 0; j < apiProducts.length; j++) {
      var q = apiProducts[j];
      if (!q) continue;
      if (String(q.name || '').trim().toLowerCase() === rowName) return q;
    }
    return null;
  }
  ...
  function inferBrandFromProduct(p) {
    if (!p) return '';
    if (p.brand) return String(p.brand).toLowerCase().trim();
    var n = (p.name || '').toLowerCase();
    var img = (p.image || '').toLowerCase();
    if (n.indexOf('chanel') !== -1 || img.indexOf('chanel') !== -1 || img.indexOf('channel') !== -1) return 'chanel';
    if (n.indexOf('dior') !== -1 || img.indexOf('dior') !== -1 || img.indexOf('doir') !== -1 || img.indexOf('diorevaa') !== -1 || img.indexOf('luky.unisex') !== -1) return 'dior';
    if (n.indexOf('boss') !== -1 || n.indexOf('hugo') !== -1 || img.indexOf('boss') !== -1) return 'boss';
    return '';
  }

  /** Home hero / flash products stay off shop grids; database catalog lists each product once. */
  function shouldListOnShop(p) {
    if (!p) return false;
    if (catalogUsesDatabase) {
      return /^[a-f\d]{24}$/i.test(String(p.id)) || String(p.id).indexOf('custom-') === 0;
    }
  ...
  function noirGetProductsForShopPage(kind, brandFilter) {
    var k = (kind || 'shopall').toLowerCase();
    if (k === 'all') k = 'shopall';
    var brand = (brandFilter || '').toLowerCase().trim();
    var out = [];
    var seen = {};
    Object.keys(ALL).forEach(function (id) {
      ...
      var dedupeKey = p.mongoProductId || (/^[a-f\d]{24}$/i.test(String(p.id)) ? p.id : id);
      if (seen[dedupeKey]) return;
      seen[dedupeKey] = true;
      if (brand && inferBrandFromProduct(p) !== brand) return;
  ...
  function noirGetProductById(id) {
    ...
    var mongoId = LEGACY_ALIAS[key];
    if (mongoId && ALL[mongoId]) return ALL[mongoId];
    return null;
  }

  function noirResolveMongoProductId(id) {
    var key = String(id || '');
    if (/^[a-f\d]{24}$/i.test(key)) return key;
    if (LEGACY_ALIAS[key]) return LEGACY_ALIAS[key];
    var p = noirGetProductById(key);
    if (!p) return key;
    if (p.mongoProductId) return String(p.mongoProductId);
    if (/^[a-f\d]{24}$/i.test(String(p.id))) return String(p.id);
    return key;
  }
  ...
  function noirGetShopProductByIndex(index) {
    ...
    return noirGetProductById('shop-' + i);
  }
  ...
  function noirProductUrl(p) {
    ...
    if (isExpressApp()) {
      var mongoId = window.noirResolveMongoProductId
        ? window.noirResolveMongoProductId(p.id)
        : null;
      if (mongoId && /^[a-f\d]{24}$/i.test(String(mongoId))) {
        return '/products/' + encodeURIComponent(mongoId);
      }
      if (p.id) return '/products/' + encodeURIComponent(String(p.id));
      if (p._id) return '/products/' + encodeURIComponent(p._id);
      if (p.shopIndex != null) return '/products/shop-' + p.shopIndex;
      return '/shop';
    }
 ...
 function normalizeImagePath(pathLike) {
  ...
  if (
    v.indexOf('/uploads/') === 0 ||
    v.indexOf('/User/Images/') === 0 ||
    v.indexOf('User/Images/') === 0 ||
    v.indexOf('http://') === 0 ||
    v.indexOf('https://') === 0
  ) {
    return v.indexOf('/') === 0 || v.indexOf('http') === 0
      ? v
      : '/' + v;
  }
  ...
  window.noirResolveMongoProductId = noirResolveMongoProductId;
  ...
  window.noirInferBrandFromProduct = inferBrandFromProduct;
  window.noirSHOP_ROWS = SHOP_ROWS;

  var catalogLoadPromise = null;

  function replaceCatalogWithApiProducts(apiProducts) {
    if (!Array.isArray(apiProducts) || !apiProducts.length) return;

    Object.keys(ALL).forEach(function (k) {
      if (k.indexOf('shop-') === 0 || /^[a-f\d]{24}$/i.test(k)) delete ALL[k];
    });
    LEGACY_ALIAS = {};
    ...
    apiProducts.forEach(function (p) {
      ...
      var rowIndex = SHOP_ROWS.findIndex(function (r) {
        return !!findApiProductForShopRow(r, [p]);
      });
      if (rowIndex >= 0) p.shopIndex = rowIndex;
      ALL[p.id] = normalizeShopGenderFields(Object.assign({}, p, { shopListing: true }));
    });

    SHOP_ROWS.forEach(function (row, index) {
      var match = findApiProductForShopRow(row, apiProducts);
      if (match && match.id) {
        LEGACY_ALIAS['shop-' + index] = String(match.id);
        if (ALL[match.id]) ALL[match.id].shopIndex = index;
      }
    });

    catalogUsesDatabase = true;
    applyAdminCatalog(ALL);
    window.noirCatalogIds = Object.keys(ALL);
    window.noirCatalogUsesDatabase = true;
    if (typeof window.dispatchEvent === 'function') {
      window.dispatchEvent(new CustomEvent('noir-catalog-updated'));
    }
  }
  ...
  function loadCatalogFromApi() {
    ...
    if (typeof fetch !== 'function') {
      catalogLoadPromise = Promise.resolve();
      return catalogLoadPromise;
    }
    catalogLoadPromise = fetch('/api/products', { credentials: 'same-origin' })
      .then(function (res) {
      ...
      .then(function (data) {
        if (data && data.success && Array.isArray(data.products)) {
          replaceCatalogWithApiProducts(data.products);
        }
      })
      .catch(function () {
  ...
  window.noirCatalogReady = loadCatalogFromApi;
  loadCatalogFromApi();
})();
