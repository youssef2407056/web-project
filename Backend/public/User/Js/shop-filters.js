...
function getShopPageKind() {
  var bodyKind = document.body && document.body.getAttribute('data-shop-kind');
  if (bodyKind) return String(bodyKind).toLowerCase();
  var path = (typeof location !== 'undefined' && location.pathname) ? location.pathname.replace(/\\/g, '/') : '';
  var file = path.split('/').pop().toLowerCase();
  if (file === 'shop' || path === '/shop') return 'shopall';
...
function genderFromCard(card) {
  var dataCategory = card.getAttribute('data-category');
  if (dataCategory) {
    var c0 = String(dataCategory).toLowerCase().trim();
    if (c0 === 'men' || c0 === 'women' || c0 === 'unisex') return c0;
  }
...
function onCollectionFilterChange() {
  ...
  var path = (typeof location !== 'undefined' && location.pathname) ? location.pathname.replace(/\\/g, '/') : '';
  var isExpress = !/\.html$/i.test(path);
  ...
  if (isExpress) {
    ...
    var target = expressMap[v];
    if (!target) return;
    var current =
      window.location.pathname +
      (window.location.search || '');
    if (current === target || (v === 'all' && (kind === 'shopall' || kind === 'all') && current === '/shop')) {
      applyFilters();
      return;
    }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!filterBackdrop) {
    ...
    filterBackdrop.addEventListener('click', closeMobileFilterPanels);
  }
  ...
  document.querySelectorAll('.filter-dropdown').forEach(function (dropdown) {
    dropdown.addEventListener('toggle', function () {
      ...
      } else if (!document.querySelector('.filter-dropdown[open]')) {
